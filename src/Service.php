...
use Flitty\Subscription\Models\SubscriptionCoupon;
use Flitty\Subscription\Models\SubscriptionType;
use Flitty\Subscription\Models\Transaction;
...
class Service
{
    ...
    {
        return config('subscription.subscription.model')::create($attributes);
    }
    ...
    public function getSubscriptions($callback = null) : EloquentCollection
    {
        ...
        $subscriptions = app(config('subscription.subscription.model'));
        if ($callback) {
    ...
    {
        $subscription = config('subscription.subscription.model')::where(
            config('subscription.subscription.own'),
            $subscriptionId
        );
        if($fullData) {
    ...
    {
        return config('subscription.subscription.model')::find($subscriptionId)->update($options);
    }
    ...
    {
        return config('subscription.subscription.model')::whereRecurringPaymentId($recurringPaymentId)->first();
    }
    ...
    {
        return config('subscription.transaction.model')::create($attributes);
    }
    ...
    {
        /** @var Model $model */
        $model =  config('subscription.coupon.model');
        return $model::all();
    }
    ...
    {
        return config('subscription.coupon.model')::where(
