...
use Flitty\Subscription\Exceptions\UserAlreadyHasTheSubscription;
use Flitty\Subscription\Models\Subscription;
use Flitty\Subscription\Providers\SubscriptionServiceProvider;
...
trait Subscriber
{
     ...
     *      @const Subscription::PROCESSED_STATUS
     *
     * @return Collection
     */
    ...
    {
        return $this->hasMany(
            config('subscription.subscription.model'),
    ...
    {
        return $this->getSubscriptions($status)->where(config('subscription.type.foreign'), $subscriptionTypeId)->first();
    }
    ...
    public function subscribeEntity(int $subscriptionTypeId, $subscriptionCouponName = null, $driver = SubscriptionServiceProvider::PAY_PAL_DRIVER) : RedirectResponse
    {
        ...
        if ($subscriptionCouponName) {
            ...
            $couponId = $subscriptionCoupon ? $subscriptionCoupon->{config('subscription.coupon.own')} : null;
        }
