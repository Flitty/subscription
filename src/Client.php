...
namespace Flitty\Subscription;

use Carbon\Carbon;
use Illuminate\Http\RedirectResponse;
use Illuminate\Support\Facades\DB;
use Flitty\Subscription\Contracts\SubscriberContract;
use Flitty\Subscription\Exceptions\CancelSubscriptionHasBeenFailed;
use Flitty\Subscription\Exceptions\InvalidResponseException;
use Flitty\Subscription\Exceptions\ReactivationSubscriptionHasBeenFailed;
use Flitty\Subscription\Exceptions\SubscriptionCallbackHasBeenFailedException;
use Flitty\Subscription\Exceptions\SubscriptionHasBeenFailedException;
use Flitty\Subscription\Exceptions\SubscriptionHasNotBeenFoundException;
use Flitty\Subscription\Exceptions\SubscriptionRedirectHasBeenFailedException;
use Flitty\Subscription\Exceptions\SuspendingSubscriptionHasBeenFailed;
use Flitty\Subscription\Exceptions\UserHasNoSubscriptionException;
use Flitty\Subscription\Models\Subscription;
...
abstract class Client
{
    ...
    public function reactivateSubscription(SubscriberContract $subscriber, int $subscriptionTypeId) : bool
    {
        ...
        return app('sub-service')->updateSubscription(
            $subscription->{config('subscription.subscription.own')},
            [
    ...
    public function suspendSubscription(SubscriberContract $subscriber, int $subscriptionTypeId) : bool
    {
        ...
        app('sub-service')->updateSubscription(
            $subscription->id,
            [
                'suspended_at' => $now,
                'status' => Subscription::SUSPENDED_STATUS
            ]
        );
        return true;
    }
    ...
    public function cmdCallback(array $request) : bool
    {
        ...
        if ($responseType) {
            ...
            if ($responseType == static::RECURRING_PAYMENT) {
                $expireAt = Carbon::now()->addMonth()->addHour()->toDateTimeString();
            ...
            $transactionAttributes = [
                config('subscription.subscription.foreign') => $subscription->{config('subscription.subscription.own')},
    ...
    public function subscriptionCallback(string $token) : bool
    {
        ...
        $expireAt = $isSuccess ? Carbon::now()->addMonth()->addHour()->toDateTimeString() : null;
        $subService->updateSubscription(
            $subscriptionId,
            [
    ...
    {
        DB::beginTransaction();
        $subscription = $subscriber->availableSubscriptions()->where(config('subscription.type.foreign'), $subscriptionTypeId)->first();
        if (!$subscription) {
            $subscription = app('sub-service')->createSubscription([
                config('subscription.coupon.foreign') => $subscriptionCouponId,
                config('subscription.type.foreign') => $subscriptionTypeId,
                config('subscription.subscriber.foreign') => $subscriber->getId(),
                'driver' => $this->driver,
            ]);
        }
        $subscription->status = static::PROCESSED_STATUS;
        $subscription->save();
        $paymentData = $this->getSubscriptionData($subscription->{config('subscription.subscription.own')});
